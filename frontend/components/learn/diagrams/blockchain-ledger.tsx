  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      <div className="flex flex-col items-center">
        ...
        <div className="flex flex-col items-center gap-0">
          {blocks.map((block, i) => (
            <div key={i} className="flex flex-col items-center">
              <div
                className={cn(
                  'w-28 sm:w-36 md:w-44 h-9 sm:h-11 rounded-lg border flex items-center justify-center text-[10px] sm:text-xs font-medium transition-all duration-500',
                  block.highlight
              ...
              {i < blocks.length - 1 && (
                <div
                  className={cn(
                    'w-px h-2 sm:h-3 bg-gradient-to-b from-border/50 via-primary/20 to-border/50 transition-opacity duration-500',
        ...
        {/* "Immutable" badge - responsive */}
        <div
          className={cn(
            'mt-6 sm:mt-8 px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl border transition-all duration-700',
            isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95',
            'bg-destructive/10 border-destructive/40'
          )}
          style={{ transitionDelay: '500ms' }}
        >
          <span className="text-[10px] sm:text-sm font-bold tracking-widest text-destructive">IMMUTABLE · NEVER EXPIRES</span>
        </div>
      </div>
    </div>
  )
