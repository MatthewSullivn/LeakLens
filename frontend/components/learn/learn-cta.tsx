...
export const LearnCTA = memo(function LearnCTA() {
  ...
  return (
    <section className="relative py-24 px-4 overflow-hidden border-t border-border/30">
      {/* Distinct background - gradient card to break up the black */}
      <div className="absolute inset-0 bg-gradient-to-b from-background via-primary/5 to-background" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_50%,oklch(0.75_0.15_195_/_0.06),transparent)]" />

      <div className="relative max-w-4xl mx-auto">
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 mb-6">
            <Shield className="w-4 h-4 text-primary" />
            <span className="text-sm font-medium text-primary">Take back control</span>
          </div>
        ...
        <div className="grid gap-6 mb-12">
          {benefits.map((item, i) => (
            <div
              key={i}
              className={cn(
                'flex gap-4 p-6 rounded-2xl border bg-card/50 backdrop-blur-sm',
