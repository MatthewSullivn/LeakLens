...
async def on_wallet_activity(event: dict):
    ...
    """
    print(f"[LeakLens] 🚨 Wallet activity detected: {event['wallet'][:10]}...")
...
async def stalker_websocket(websocket: WebSocket):
    ...
    print(f"[LeakLens] 👁️  Monitor client connected (total: {len(active_stalker_connections)})")
    ...
    try:
        ...
        print("[LeakLens] Monitor client disconnected")
    except Exception as e:
...
async def shutdown_event():
    ...
    if STALKER_SUPPORTED:
        print("[LeakLens] Shutting down monitoring services...")
