#!/usr/bin/env python3
"""
LeakLens Live Monitoring Service
WebSocket-based real-time wallet monitoring for EVM and Solana chains
Part of LeakLens - See What Your Wallet Leaks
...
class WalletStalker:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.connected = True
            print(f"[LeakLens] Connected to {self.chain} WebSocket")
            
            # Start listening loop
            self.connection_task = asyncio.create_task(self._listen_loop())
            
        except Exception as e:
            print(f"[LeakLens] Connection failed: {e}")
            self.connected = False
            raise
    ...
    async def disconnect(self):
        ...
        if self.websocket:
            ...
            print(f"[LeakLens] Disconnected from {self.chain}")
    ...
    async def watch_wallet(self, wallet_address: str):
        ...
        if wallet_address in self.watched_wallets:
            print(f"[LeakLens] Already watching {wallet_address[:10]}...")
            return
        ...
        try:
            ...
            if "result" in result:
                ...
                print(f"[LeakLens] 👁️  Now watching: {wallet_address[:10]}... (sub: {sub_id})")
                return True
            else:
                print(f"[LeakLens] Subscription failed: {result}")
                return False
                
        except Exception as e:
            print(f"[LeakLens] Failed to watch wallet: {e}")
            return False
    ...
    async def unwatch_wallet(self, wallet_address: str):
        ...
        if sub_id and self.websocket:
            ...
            try:
                ...
                print(f"[LeakLens] Unsubscribe error: {e}")
        ...
        print(f"[LeakLens] Stopped watching: {wallet_address[:10]}...")
    ...
    async def _listen_loop(self):
        ...
        try:
            ...
            print("[LeakLens] Connection closed")
            self.connected = False
        except asyncio.CancelledError:
            print("[LeakLens] Listen loop cancelled")
        except Exception as e:
            print(f"[LeakLens] Listen loop error: {e}")
            self.connected = False
    ...
    async def _process_message(self, message: str):
        ...
        try:
            ...
            print(f"[LeakLens] Message processing error: {e}")
    ...
    async def _handle_wallet_activity(self, wallet: str, tx_hash: str, block_number: str, event_data: dict):
        ...
        # Log detection
        print(f"[LeakLens] 🚨 TARGET ACTIVE! {wallet[:10]}... | Tx: {tx_hash[:10]}... | Block: {block_number}")
        ...
        if self.on_activity:
            ...
            try:
                ...
                print(f"[LeakLens] Callback error: {e}")
...
class SolanaWalletStalker:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.connected = True
            print(f"[LeakLens] Connected to Solana WebSocket")
            
            # Start listening loop
            self.listen_task = asyncio.create_task(self._listen_loop())
            
        except Exception as e:
            print(f"[LeakLens] Solana connection failed: {e}")
            self.connected = False
            raise
    ...
    async def disconnect(self):
        ...
        if self.websocket:
            try:
                ...
                print(f"[LeakLens] Error closing Solana connection: {e}")
            print(f"[LeakLens] Disconnected from Solana")
    ...
    async def watch_wallet(self, wallet_address: str):
        ...
        try:
            ...
            print(f"[LeakLens] 👁️  Now watching Solana: {wallet_address[:10]}...")
            return True
                
        except Exception as e:
            print(f"[LeakLens] Failed to watch Solana wallet: {e}")
            return False
    ...
    async def unwatch_wallet(self, wallet_address: str):
        ...
        print(f"[LeakLens] Stopped watching Solana: {wallet_address[:10]}...")
    
    async def _listen_loop(self):
        """
        Continuous loop that listens for incoming WebSocket messages from Solana.
        """
        print("[LeakLens] Solana listen loop started")
        try:
            while self.connected and self.websocket:
                try:
                    # Receive messages from Solana WebSocket
                    messages = await self.websocket.recv()
                    print(f"[LeakLens] Received {len(messages) if messages else 0} Solana messages")
                    ...
                    print("[LeakLens] Solana listen loop cancelled")
                    break
                except Exception as e:
                    print(f"[LeakLens] Solana listen error: {e}")
                    await asyncio.sleep(1)
                    
        except Exception as e:
            print(f"[LeakLens] Solana listen loop error: {e}")
            self.connected = False
    ...
    async def _process_message(self, message):
        ...
        try:
            ...
            if self._debug_count < 3:
                print(f"[LeakLens] DEBUG #{self._debug_count + 1} - Full message:")
            ...
            if method == "logsNotification":
                print("[LeakLens] ✅ Logs notification received!")
                params = data.get("params", {})
                result = params.get("result", {})
                ...
                if not signature:
                    print("[LeakLens] No signature in notification")
                    return
                
                print(f"[LeakLens] Signature: {signature[:10]}... for {len(self.watched_wallets)} watched wallets")
                ...
                for wallet in self.watched_wallets:
                    ...
                    print(f"[LeakLens] Triggering activity for wallet: {wallet[:10]}...")
            ...
            print(f"[LeakLens] Solana message processing error: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def _handle_wallet_activity(self, wallet: str, signature: str, log_data: dict):
        ...
        # Log detection
        print(f"[LeakLens] 🚨 SOLANA TARGET ACTIVE! {wallet[:10]}... | Tx: {signature[:10]}...")
